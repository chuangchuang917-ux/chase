...
## 33. 📱 手機版 (app_mobile.py) 個股大戶增幅數值與週數顯示排版優化

* **發生問題**：
  使用者反應當條件設定「大戶累積增幅門檻」為 3% 與 5% 時，畫面上均顯示「千張大戶週變動：3」，導致使用者誤將「連買 3 週」的 3 視為「累積增幅 3%」，進而認為 5% 門檻的過濾器失效。
* **問題根因**：
  手機版卡片 UI 未加上週數單位（僅輸出原始整數 3），且未將策略背後實際過濾依據的百分比 holder_growth_pct 欄位呈現給使用者驗證。
* **執行修正**：
  在 pp_mobile.py 中，將千張與 400 張大戶的週變動數值明確格式化為 +N 週 (連續增加) 與 -N 週 (連續減少)。同時，在卡片版面中新增 累積增幅：+X.XX% 欄位，徹底消除數據定義上的歧義。
